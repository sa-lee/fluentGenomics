...
enrichment of peaks in DE genes compared to other genes. 
We see that the relative enrichment increases  for a 'larger' LFC:

```{r}
origin_peak_lfc %>%
  as.data.frame() %>%
  select(-origin) %>%
  summarize_all(.funs = ~Reduce(function(x,y) y/x, .))
```
