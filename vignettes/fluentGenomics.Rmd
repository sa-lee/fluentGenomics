...
(ref:maplot) Visualization of *DESeq2* results as a mean-abundance (MA) plot. Genes that have an adjusted *p-value* below 0.01 are colored red.
...
they represent a gene that is DE or not. Note the x-axis is on a log-scale.
...
of human donors with interferon (IFN) gamma, *Salmonella* infection, or both
...
tied to genomic ranges, e.g. shifts, windows, overlaps, etc. The *plyranges*
package is described by @Lee2019, and leverage underlying range operations
described by @granges. The second package described by @Love2019-tximeta is
...
used to generate the `coldata` data.frame for the RNA-seq experiment:
...
For all variables except for the `origin` column we divide the first rows
