...
QTL in naive cells, and futher hypothesize about the nature and role
...
(we select based on an un-adjusted p-value larger than 0.1 for example).
...
<!-- The above code makes use of the *dplyr* framework, but is admittedly a -->
<!-- bit complicated to read if one is not familiar with `.funs` or -->
<!-- `Reduce`. We can also perform a simple ratio of two rows of a  -->
<!-- matrix, giving the same result. The above code however allows us to -->
<!-- perform the operation as a single composition of verbs, rather than -->
<!-- needing to save an intermediate object. -->
...
apply it over possible LFC thresholds of interest. We choose a grid of
...
Now we can expand these list-columns into a long *GRanges* object (since
we have used . To compute the peak and gene counts
...
the enrichment. Then we can visualise how the peak enrichment changes of 
DE genes relative to other genes using a line chart:
