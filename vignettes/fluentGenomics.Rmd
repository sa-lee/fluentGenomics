...
is *not* large. We perform this test by specifying an LFC threshold
and an alternative hypothesis (`altHypothesis`) that the LFC is less
than the threshold in absolute value. To visualize the result of this
test, you can run `results` without `format="GRanges"`, and pass this
object to `plotMA` as before. We label these genes as `other_genes`
and later as "non-DE genes", for comparison with our `de_genes` set.
...
In the discussion, I would spend less time describing the packages
again and more on talking about how cool it is that so many decoupled
packages, both from Bioconductor and the tidyverse, can interoperate
to enable seamless end-to-end workflows that are far too specialized
to be solved by a single, monolithic tool.
...
*plyranges* is readable and clear; in most cases the code we have 
