...
```{r cache-path, eval = retrieve_cache}
library(fluentGenomics)
...
```{r read-cache, eval = retrieve_cache}
...
count that meets the DA LFC threshold.
...
The peaks are computed for each threshold value by applying the function
`count_if_above_threshold()` on a *GRanges* object that has been grouped
by the gene, origin and the number of resamples variables.
...
# Mike: I'm concerned about how long this takes, can we make it more efficient?
...
origin_peak_all_thresholds %>%
  as.data.frame() %>%
  tidyr::gather(type, count, gene_count:peak_count) %>%
  mutate(type = sub("_count","",type)) %>%
