...
it is not present will create the cache and generate the file for you.
We then read the file and assign to an object called `atac`.
...
```{r}
ggplot(data = data.frame(rmu = rowMeans(assay(atac)), 
                         rvar = rowVars(assay(atac))),
       aes(x = rmu, y = sqrt(rvar))) + geom_hex()
```
...
```{r}
da_peaks <- atac_peaks %>% filter(da_padj < .01)
```
...
of it's verbs for *GRanges* objects. Here we can 
