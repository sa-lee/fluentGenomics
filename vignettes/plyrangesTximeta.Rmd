...
we are estimating log2 fold changes of IFNg stimulated cell lines against naive cell lines. We are interested in log fold changes greater than 1 at a false discovery rate at of 1%. to test for  for 
...
called `gene_id` from the rownames of the results. Each
...
```{r eval=FALSE}
atac_mat <- read_tsv(here::here("data-raw","ATAC_cqn_matrix.txt.gz")))
```

We also read in the sample metadata:

```{r eval=FALSE}
atac_coldata <- read_tsv(here::here("data-raw", "ATAC_sample_metadata.txt.gz")) %>% 
 select(
    sample_id,
    donor,
    condition = condition_name
...
```{r eval=FALSE}
peaks_df <- read_tsv(
  here::here("data-raw","ATAC_peak_metadata.txt.gz"),
...
In *plyranges*, we can select ranges by their position using *slice*. Using this we can downsample the number of "other" genes to the number of DE genes: 

```{r}
other_genes <- other_genes %>%
  slice(sample.int(n(), length(de_genes)))
```
