...
# ATAC seq data. Modified from the BiocFileCache vignette

#' Download, retrieve and cache macrophage ATAC seq results
#' 
#' @param verbose Provide progress bar for downloading and update the cache?
#' (Default is FALSE)
#' 
...
cache_atac_se <- function(verbose = FALSE) {
  ...
  if (BiocFileCache::bfccount(res) == 0L) {
    ...
    atac_coldata <- readr::read_tsv(tmp_coldata, progress = verbose)
    ...
    peaks_gr <- plyranges::as_granges(peaks_df, seqnames = chr)
    peaks_gr <- dplyr::select(peaks_gr, peak_id = gene_id)
    peaks_gr <- plyranges::set_genome_info(peaks_gr, genome = "GRCh38")
    ...
    atac_mat <- readr::read_tsv(mat_file, 
                                skip = 1, 
    ...
    saveRDS(se, BiocFileCache::bfcnew(bfc, resource, ext = ".rds"))
  } 
