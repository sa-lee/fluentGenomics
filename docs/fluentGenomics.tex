...
K Alasoo et al. (2018), a study that involved treatment of macrophage cell lines from a number
of human donors with interferon (IFN) gamma, \emph{Salmonella} infection, or both
treatments combined. K Alasoo et al. (2018) examined gene expression and chromatin
accessibility in a subset of 86 successfully differentiated induced pluripotent
stem cells (iPSC) lines, and examined baseline quantitative trait loci (QTL)
and response QTL for chromatin accessibility and gene expression. The authors
found that many of the stimulus-specific expression QTL were already detectable
as chromatin QTL in naive cells, and further hypothesize about the nature and
role of transcription factors implicated in the response to stimulus.

In this workflow, we will perform a much simpler analysis than the one found in
K Alasoo et al. (2018), using their publicly available RNA-seq and ATAC-seq data (ignoring the
...
{\centering \includegraphics[width=\textwidth]{/Library/Frameworks/R.framework/Versions/3.6/Resources/library/fluentGenomics/vignettes/workflow} 
...
to compare differentially expressed genes to non differentially expressed
...
The first package is be used to perform easily-readable transformations of data
tied to genomic ranges, e.g.~shifts, windows, overlaps, etc. The \emph{plyranges}
package is described by Lee, Cook, and Lawrence (2019), and leverage underlying range operations
described by W. A. P. Lawrence Michael AND Huber (2013). The second package described by M. I. Love et al. (2019) is
used to read RNA-seq quantification into R/Bioconductor, such that the
transcript ranges and their provenance are automatically attached to an object
containing the quantification data and the differential expression results.
...
samples, a subset of the RNA-seq samples generated and analyzed by K Alasoo et al. (2018). The
paired-end reads were quantified using \emph{Salmon} (R. Patro et al. 2017), using the Gencode 29
...
by Kaur Alasoo and Gaffney (2017). This object contains all 145 ATAC-seq samples across all
experimental conditions as analyzed by K Alasoo et al. (2018). The data can be also be
...
the \emph{BiocFileCache} package (Shepherd and Morgan 2019).
...
this file into R (Wickham et al. 2019). We will see later that \emph{plyranges} extends these
...
typically not have the `gz' ending if used from \texttt{salmon} directly. One other
...
methods of Soneson, Love, and Robinson (2015).
...
the following tab-delimited files from Kaur Alasoo and Gaffney (2017):
...
used to generate the \texttt{coldata} data.frame for the RNA-seq experiment:
...
The ATAC-seq counts have already been normalized with \emph{cqn} (Hansen, Irizarry, and Wu 2012) and
...
workflows in Bioconductor see M. I. Love et al. (2016) and Law et al. (2018).
...
\caption{Visualization of \emph{DESeq2} results as a mean-abundance (MA) plot. Genes that have an adjusted \emph{p-value} below 0.01 are colored red.}\label{fig:ma-plot}
\end{figure}
...
\emph{GRanges} object of differential peaks from the ATAC-seq data in K Alasoo et al. (2018).

The code chunks for the remainder of this section are not run.

For assessing differential accessibility, we run \emph{limma} (G. K. Smyth 2004), and
...
for performing transformations of genomic data (Lee, Cook, and Lawrence 2019). Computations
...
(W. A. P. Lawrence Michael AND Huber 2013).
...
##         seqnames              ranges strand |            gene_id
##            <Rle>           <IRanges>  <Rle> |        <character>
##     [1]    chr22   30522799-30546682      - |  ENSG00000214491.8
##     [2]    chr16   31432593-31443160      - |  ENSG00000176723.9
##     [3]    chr15   70829130-70854159      - | ENSG00000166173.10
##     [4]    chr16   75228187-75268053      - | ENSG00000050820.16
##     [5]     chr5       418703-419109      + |  ENSG00000250645.1
##     ...      ...                 ...    ... .                ...
##   [745]     chr2 169127109-169362685      - | ENSG00000081479.13
##   [746]     chr3   33496245-33718356      - | ENSG00000163539.16
##   [747]    chr16   81517824-81520617      + |  ENSG00000279476.1
##   [748]    chr12   10824960-11171608      - |  ENSG00000231887.7
##   [749]     chrX   63343227-63561071      - |  ENSG00000235437.7
##                   de_log2FC           de_padj
##                   <numeric>         <numeric>
##     [1]  -0.617179118187071 0.344699105358044
##     [2]   0.294867582388685 0.551257713325106
##     [3] 0.00320582304477657 0.996259965931837
##     [4]   0.354877253304737 0.523866126057675
##     [5]   0.285154290606574 0.707150641827777
##     ...                 ...               ...
##   [745] -0.0222658626167039 0.986002302490244
##   [746] -0.0293166616928116 0.904144020961804
##   [747]   0.170661533153461 0.809288209488289
##   [748]  0.0387332579224126 0.949684393107519
##   [749]   -0.24302888160628 0.325230189643086
##   -------
##   seqinfo: 25 sequences (1 circular) from hg38 genome
\end{verbatim}
...
number or DA peaks decrease as we increase the DA LFC threshold (as expected).
...
For all variables except for the \texttt{origin} column we divide the first rows
values by the second row, which will be the enrichment of peaks in DE genes
compared to other genes. This requires us to reshape the summary table to long
form back to wide form using the \emph{tidyr} package. First we pivot the results of
the peak\_count columns into name-value pairs, then pivot again to place values
...
row will contain the peak counts for all of the thresholds for a within gene,
...
the enrichment. We visualize how the peak enrichment changes of DE genes
...
change between non-DE genes compared to DE genes as the absolute DA LFC
...
{\centering \includegraphics{/Users/lee.s/fluentGenomics/docs/fluentGenomics_files/figure-latex/linechart2-1} 
...
they represent a gene that is DE or not. Note the x-axis is on a log-scale.}\label{fig:linechart2}
\end{figure}
...
both the DE and DA peaks. We could also have computed variance in addition to
...
This article and the analyses were performed with R (R Core Team 2019) using the
\emph{rmarkdown} (Allaire et al. 2019), and \emph{knitr} (Xie 2019, 2015) packages.
...
##  [1] ggplot2_3.2.1.9000          plyranges_1.7.7            
...
## [19] fluentGenomics_0.0.3        rmarkdown_2.0              
## 
...
##  [43] lifecycle_0.1.0          ensembldb_2.10.2         XML_3.98-1.20           
...
Law, Charity W, Monther Alhamdoosh, Shian Su, Xueyi Dong, Luyi Tian, Gordon K Smyth, and Matthew E Ritchie. 2018. ``RNA-seq Analysis Is Easy as 1-2-3 with Limma, Glimma and edgeR.'' \emph{F1000Res.} 5 (1408). F1000 Research Limited:1408. \url{https://doi.org/10.12688/f1000research.9005.3}.

\leavevmode\hypertarget{ref-granges}{}%
Lawrence, Wolfgang AND Pagès, Michael AND Huber. 2013. ``Software for Computing and Annotating Genomic Ranges.'' \emph{PLOS Computational Biology} 9 (8). Public Library of Science:1--10. \url{https://doi.org/10.1371/journal.pcbi.1003118}.
...
Love, Michael I, Simon Anders, Vladislav Kim, and Wolfgang Huber. 2016. ``RNA-Seq Workflow: Gene-Level Exploratory Analysis and Differential Expression.'' \emph{F1000Res.} 4 (1070). F1000 Research Limited:1070. \url{https://doi.org/10.12688/f1000research.7035.2}.
